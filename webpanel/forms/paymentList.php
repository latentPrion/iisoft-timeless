...
<form method="post" action="../processing/paymentView.php">
<span class="normal">
<?php
	$resultHandle = odbc_exec($dbHandle, "select * from payment INNER JOIN customer ON payment.customerId = customer.id");
?>
...
?></td>
	...
	<tr>
		<td colspan="4">payment listing.</td>
	</tr>
	<tr>
		<td>Id:</td>
		<td>Customer Name:</td>
...
?>
	<tr>
		...
		<td><?php echo "$" . odbc_result($resultHandle, "amount"); ?></td>
		<td>
			<?php
				switch ($_REQUEST['mode'])
				{
				default:
					echo "<a href=\"processing/paymentDetails.php?id=" . odbc_result($resultHandle, "id") . "\">View details</a>";
					break;

				case 'edit':
					echo "<a href=\"form.php?table=payment&form=Edit&id=" . odbc_result($resultHandle, "id") . "\">Edit payment</a>";
					break;

				case 'remove':
					echo "<a href=\"processing/paymentRemove.php?id=" . odbc_result($resultHandle, "id") . "\">Delete payment</a>";
					break;
				};
			?>
		</td>
